...
class NeuralNetwork(object):
    ...
    def backpropagation(self, final_outputs, hidden_outputs, X, y, delta_weights_i_h, delta_weights_h_o):
        ...
        #hidden_outputs.T.dot(error)
        
        hidden_error_term = output_error_term.dot(self.weights_hidden_to_output.T) * hidden_error
        #print(X)
        # Weight step (input to hidden)
        delta_weights_i_h += X[:,None].dot(np.array(hidden_error_term, ndmin=2))
        # Weight step (hidden to output)
        delta_weights_h_o += hidden_outputs[:,None].dot(np.array(output_error_term, ndmin=2))
